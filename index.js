...
setInterval(function(){
    var d= new Date();
    if(reading[0]>=7){ //Change this value afterwards, as when pipe closes only small flow will be there...
        Store.create({flow:reading[0],level:reading[1],vol:reading[2],date:d.getDate()+"-"+(d.getMonth()+1)+"-"+d.getFullYear(),event:event,time: (d.getHours())+":"+(d.getMinutes())+":"+(d.getSeconds())},function(err,obj){ //Change here reading[0]
            if(err){
                console.log(err);
            }
            else{
